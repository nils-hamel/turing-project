...
ml_h_flat   = ml_args.width * ml_args.width * ml_args.width
...
ml_p_w1 = tf.Variable( tf.random_normal( [ ml_h_flat, ml_h_hidden ], stddev = 0.05 ) )
ml_p_w2 = tf.Variable( tf.random_normal( [ ml_h_hidden, ml_h_flat ], stddev = 0.05 ) )

# network parameter : biases #
ml_p_b1 = tf.Variable( tf.zeros( [ ml_h_hidden ] ) )
ml_p_b2 = tf.Variable( tf.zeros( [ ml_h_flat   ] ) )
...
ml_g_input = tf.placeholder( tf.float32, [ None, ml_h_width, ml_h_width, ml_h_width ] )

# network topology : input layer #
ml_g_input_flat = tf.reshape( ml_g_input, [ -1, ml_h_flat ] )
...
# network topology : output layer #
ml_g_output = tf.reshape( ml_g_output_flat, [ -1, ml_h_width, ml_h_width, ml_h_width ] )
...
# network topology : input layer #
ml_s1_input = tf.placeholder( tf.float32, [ None, ml_h_width, ml_h_width, ml_h_width ] )

# network topology : input layer #
ml_s1_input_flat = tf.reshape( ml_s1_input, [ -1, ml_h_flat ] )

# network topology : output layer #
ml_s1_output = tf.nn.sigmoid( tf.add( tf.matmul( ml_s1_input_flat, ml_p_w1 ), ml_p_b1 ) )

# network topology : input layer #
ml_s2_input = tf.placeholder( tf.float32, [ None, ml_h_hidden ] )

# network topology : output layer #
ml_s2_output_flat = tf.nn.sigmoid( tf.add( tf.matmul( ml_s2_input, ml_p_w2 ), ml_p_b2 ) )

# network topology : output layer #
ml_s2_output = tf.reshape( ml_s2_output_flat, [ -1, ml_h_width, ml_h_width, ml_h_width ] )
...
ml_o_loss = tf.reduce_mean( tf.pow( tf.subtract( ml_g_input_flat, ml_g_output_flat ), 2 ) )
...
# script mode #
    ...
    for ml_export in range( ml_data.shape[0] ):
        ...
        # export decoded raster #
        td.ml_data_raster_save( ml_decode[ml_export], ml_args.output + '/raster-{:06d}.ras'.format( ml_export + ml_args.start ) )
