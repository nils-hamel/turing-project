...
ml_g_hidden_a = tf.asinh( tf.add( tf.matmul( ml_g_input, ml_p_w1 ), ml_p_b1 ) )

# network topology : hidden layer #
ml_g_hidden_b = tf.asinh( tf.add( tf.matmul( ml_g_hidden_a, ml_p_w2 ), ml_p_b2 ) )

# network topology : hidden layer #
ml_g_hidden_c = tf.asinh( tf.add( tf.matmul( ml_g_hidden_b, ml_p_w3 ), ml_p_b3 ) )

# network topology : output layer #
ml_g_output = tf.asinh(  tf.add( tf.matmul( ml_g_hidden_c, ml_p_w4 ), ml_p_b4 ) )
...
# network topology : output layer #
ml_s1_hidden = tf.asinh( tf.add( tf.matmul( ml_s1_input, ml_p_w1 ),ml_p_b1 ) )

# network topology : output layer #
ml_s1_output = tf.asinh( tf.add( tf.matmul( ml_s1_hidden, ml_p_w2 ),ml_p_b2 ) )

# network topology : input layer #
ml_s2_input = tf.placeholder( tf.float32, [ None, ml_h_hidden2 ] )

# network topology : output layer #
ml_s2_hidden = tf.asinh( tf.add( tf.matmul( ml_s2_input, ml_p_w3 ), ml_p_b3 ) )

# network topology : output layer #
ml_s2_output = tf.asinh( tf.add( tf.matmul( ml_s2_hidden, ml_p_w4 ), ml_p_b4 ) )
...
if ( ( ml_args.mode == 'train' ) or ( ml_args.mode == 'retrain' ) ):
    ...
    # format data #
    ml_data = auto_data.ml_data_format_central( ml_data )
...
# script mode #
    ...
    # format image #
    ml_data = auto_data.ml_data_format_central_invert( ml_data )
    ml_auto = auto_data.ml_data_format_central_invert( ml_auto )
...
# script mode #
    ...
    # format image #
    ml_decode = auto_data.ml_data_format_central_invert( ml_decode )
