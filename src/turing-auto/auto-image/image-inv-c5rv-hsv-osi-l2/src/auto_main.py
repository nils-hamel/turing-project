...
ml_p_b3 = tf.Variable( tf.zeros( [ ml_h_input   ] ) )
...
ml_g_input = tf.placeholder( tf.float32, [ None, ml_h_width, ml_h_width, 1 ] )
...
ml_g_output = tf.reshape( ml_g_output_flat, [ -1, ml_h_width, ml_h_width, 1 ] )
...
# network topology : input layer #
ml_s1_input = tf.placeholder( tf.float32, [ None, ml_h_width, ml_h_width, 1 ] )
...
# network topology : output layer #
ml_s2_output = tf.reshape( ml_s2_output_flat, [ -1, ml_h_width, ml_h_width, 1 ] )
...
if ( ( ml_args.mode == 'train' ) or ( ml_args.mode == 'retrain' ) ):
    ...
    # reshape data #
    ml_data = ml_data.reshape( -1, ml_args.width, ml_args.width, 1 )
