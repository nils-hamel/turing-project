...
ml_p_w1 = tf.Variable( tf.random_normal( [ 5, 5, 1, ml_h_hidden ], stddev=0.05 ) )
ml_p_w2 = tf.Variable( tf.random_normal( [ 5, 5, ml_h_hidden, 1 ], stddev=0.05 ) )

# network parameter : biases #
ml_p_b1 = tf.Variable( tf.zeros( [ ml_h_hidden ] ) )
ml_p_b2 = tf.Variable( tf.zeros( [ 1 ] ) )
