...
ml_g_hidden_a = tf.tanh( tf.add( tf.matmul( ml_g_input, ml_p_w1 ), ml_p_b1 ) )

# network topology : hidden layer #
ml_g_hidden_b = tf.tanh( tf.add( tf.matmul( ml_g_hidden_a, ml_p_w2 ), ml_p_b2 ) )

# network topology : hidden layer #
ml_g_hidden_c = tf.tanh( tf.add( tf.matmul( ml_g_hidden_b, ml_p_w3 ), ml_p_b3 ) )

# network topology : output layer #
ml_g_output = tf.tanh(  tf.add( tf.matmul( ml_g_hidden_c, ml_p_w4 ), ml_p_b4 ) )
...
# network topology : output layer #
ml_s1_hidden = tf.tanh( tf.add( tf.matmul( ml_s1_input, ml_p_w1 ),ml_p_b1 ) )

# network topology : output layer #
ml_s1_output = tf.tanh( tf.add( tf.matmul( ml_s1_hidden, ml_p_w2 ),ml_p_b2 ) )

# network topology : input layer #
ml_s2_input = tf.placeholder( tf.float32, [ None, ml_h_hidden2 ] )

# network topology : output layer #
ml_s2_hidden = tf.tanh( tf.add( tf.matmul( ml_s2_input, ml_p_w3 ), ml_p_b3 ) )

# network topology : output layer #
ml_s2_output = tf.tanh( tf.add( tf.matmul( ml_s2_hidden, ml_p_w4 ), ml_p_b4 ) )
